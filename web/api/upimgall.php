    ...
    if(mysqli_num_rows($rowretval) !== 1){ 
        ...
        $query = $conn->query($rowsql);
                ...
                if ($ustatus==4||$ustatus==3||$ustatus==2){
                    ...
                    $text=strip_tags($_POST['text']);//相册数组（按英文逗号分割的字符串）
                    ...
                    $typeimg=['jpg', 'jpeg', 'gif', 'png', 'webp', 'svg','bmp','ico','avif'];//允许的图片后缀

                    if(!empty($text)){
                        $textarr=explode(",",$text);//将相册转为英文逗号分割的数组
                        //循环每一张图片地址，判断数组每一项的后缀是否都是图片
                        for ($i=0;$i<count($textarr);$i++){//循环每一项
                            if (!startsWithUploadOrHttp($textarr[$i])){//判断是否是upload开头的url
                                echo 405;
                                exit();
                            }
                            if (!in_array(pathinfo($textarr[$i], PATHINFO_EXTENSION), $typeimg)){//判断是否是有效的后缀
                                echo 404;
                                exit();
                            }
                        }
            ...
            $rowif=2;//文章类型，1图文，2相册，3视频
            $newsql = "INSERT INTO ppz_row (rowtexe, rowbigtext, rowtop, rowfl, rowtag, rowadmin, rowcp, rowcpurl, rowdw, rowdwgold, rowvip, rowif, rowimg, rowyes, videotext,rowdwif,vorimg,vorimg_log,videotexttop) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?)";
            $stmt = mysqli_prepare($conn, $newsql);//预处理
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "sssssssssssssssssss", $title, $textup, $top, $if, $newtag, $uid, $cp, $cpurl, $dowarr, $dow, $vip,$rowif, $img, $rowyes, $content,$dowif,$imageseye,$vipimageseye,$txttop);
