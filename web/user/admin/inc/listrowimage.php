<?php
if ($adminyeslist===200 && ($allvip==4||$allvip==3||$allvip==2)  && !is_null($ppzusername) && $rlit_if==2){
    echo '
    <div class="newword">
    ...
    echo '</select>
            ...
            <input type="file" name="files[]" id="arrimgfileInput" multiple accept="image/jpeg,image/png,image/gif,image/webp" style="display: none;" />
            <div class="newimageall" id="xcdesss"><div id="percentage"></div><div class="divvvvimg"><span class="letter1em">相册编辑：</span></div><div id="imgeye"></div></div>
            <textarea name="rowtext" id="rowtext">'.$rlit_vt.'</textarea>

            <div class="newword-title"><span class="letter1em">置顶：</span>
            <input type="radio" id="nawrowtop" name="rowtop" value="1" value="1" '.$rlit_top1.' /><label for="nawrowtop">无</label>
            <input type="radio" id="rowtop" name="rowtop" value="2" value="1" '.$rlit_top2.'/><label for="rowtop">置顶</label>
            <input type="radio" id="rowtop2" name="rowtop" value="3" value="1" '.$rlit_top3.'/><label for="rowtop2">热门</label>
            <input type="radio" id="rowtop3" name="rowtop" value="4" value="1" '.$rlit_top4.'/><label for="rowtop3">精华</label>
                <div class="imgtexttop"><span>摘要显示位置：</span>
                <input type="radio" id="imgtexttop" name="imgtexttop" value="2" '.$text_top2.'/><label for="imgtexttop">内容上方</label>
                <input type="radio" id="imgtextbottom" name="imgtexttop" value="1" '.$text_top1.'/><label for="imgtextbottom">内容下方</label></div>
            </div>

            <div class="newword-title" style="justify-content: space-between;">
                        <span>游客可见前：<input type="number" min="0" max="999999999" name="imageseye" id="imageseye" style="margin-right: 10px;" value="'.$imageseye.'" />张图片</span>
                        <span style="border-left: 2px solid var(--border-color);padding-left:45px;">
                            登录可见前：<input type="number" min="0" max="999999999" name="vipimageseye" id="vipimageseye" style="margin-right: 10px;" value="'.$vipimageseye.'" />张图片
                        </span>
                        <i>*值为0则表示不限制。</i>
            </div>
        ...
        <script src="/style/js/editarrimg.js" type="text/javascript"></script>
    </div>';
    echo '
    ...
    if(mysqli_num_rows($linkretvalyb) >= 0){
        //将|替换为,
        $rlit_text = str_replace("|",",",$rlit_text);
        echo '<script type="text/javascript">var listfllinkid="'.$listfllinkid.'";var listflid="'.$rlit_fl.'";var imgarr="'.$rlit_text.'";var rowid="'.$listid.'"</script>';
