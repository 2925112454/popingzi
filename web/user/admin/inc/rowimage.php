<?php
if ($admin==1 && $typeuser==12 && ($allvip==4||$allvip==3||$allvip==2)  && !is_null($ppzusername) && $typeyes==2){
echo '<div class="user-h1"><span><i class="fa fa-plus-circle"></i>发布相册</span><a href="?type=12"><i class="fa fa-angle-double-left" aria-hidden="true"></i>返回类型</a></div>';
echo '
<div class="newword">
...
echo '</select>
        ...
        <div class="newword-title"><span class="letter1em">相册：</span><input type="text" name="rowimgarr" id="rowimgarr" /><a id="newarrimg"><i class="fa fa-plus"></i>添加图片</a><i>* 填写url地址添加图片或留空状态下点击按钮上传图片。</i></div>
        <input type="file" name="files[]" id="arrimgfileInput" multiple accept="image/jpeg,image/png,image/gif,image/webp,image/avif" style="display: none;" />
        <div class="newimageall" id="xcdesss"><div id="percentage"></div><div class="divvvvimg"><span class="letter1em">预览：<i>单图限1MB;单次上传最多20张图片,且不能超20MB。图片宽度若超3600px会被压缩。</i></span><a id="upbutton">开始上传</a></div><div id="imgeye"></div></div>
        ...
        <div class="newword-title" style="justify-content: space-between;">
            <span>游客可见前：<input type="number" min="0" max="999999999" name="imageseye" id="imageseye" style="margin-right: 10px;" value="0" />张图片</span>
            <span style="border-left: 2px solid var(--border-color);padding-left:45px;">
                登录可见前：<input type="number" min="0" max="999999999" name="vipimageseye" id="vipimageseye" style="margin-right: 10px;" value="0" />张图片
            </span>
            <i>*值为0则表示不限制。</i>
        </div>
    ...
    <script src="/style/js/arrimg.js" type="text/javascript"></script>
</div>';
echo '
