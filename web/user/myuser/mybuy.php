...
if (empty($allnameid) || !isset($allnameid) || !is_numeric($allnameid) || $allnameid < 1 ||
    ...
    !isset($typeuser) || empty($typeuser) || $typeuser != 11) {
    if (!headers_sent()) {
        ob_clean();
...
// 获取购买数组
$collect_sql = "SELECT urowyes FROM ppz_newusername WHERE uid = $allnameid";
...
if($collect_retval && mysqli_num_rows($collect_retval) == 1){
    ...
    $collect_array = $collect_query['urowyes'];
} else {
    $collect_array = "";
}
...
if ($p == 1) {
    ...
    $pageindex = '<a class="page-button nocopy" href="?type=11">首页</a>'; // 首页按钮
}

if ($p == $total_pages || $total_pages < 1) {
    $pagebody = '<a class="page-no-button nocopy">尾页</a>';
} else {
    $pagebody = '<a class="page-button nocopy" href="?type=11&p='.$total_pages.'">尾页</a>';
}

if ($total_pages > 1 && $p < $total_pages) {
    $exit = $p + 1;
    $pageexit = '<a class="page-button nocopy" href="?type=11&p='.$exit.'">下一页</a>';
} else {
...
if ($p <= $total_pages && $p > 1) {
    $exitup = $p - 1;
    $pageup = '<a class="page-button nocopy" href="?type=11&p='.$exitup.'">上一页</a>';
} else {
    $pageup = '<a class="page-no-button nocopy">上一页</a>';
}

echo'
<div class="user-h1 myuser">我的购买</div>
...
if (!empty($articles)) {
    ...
    echo '<div class="empty-collection">您还没有购买任何内容</div>';
}
