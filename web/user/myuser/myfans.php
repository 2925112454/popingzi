...
if (empty($allnameid) || !isset($allnameid) || !is_numeric($allnameid) || $allnameid < 1 ||
    ...
    !isset($typeuser) || empty($typeuser) || $typeuser != 6) {
    if (!headers_sent()) {
        ob_clean();
...
echo '<div class="user-h1 myuser">我的粉丝</div>';
// 获取粉丝数据库
$fans_arr=[];
$fans_sql = "SELECT usvip FROM ppz_folus WHERE usuename = $allnameid";
...
if($fans_retval && mysqli_num_rows($fans_retval) > 0){
    while ($fans_query = mysqli_fetch_assoc($fans_retval)){
        $fans_arr[] = $fans_query['usvip'];
    }
}
$fans_arr = array_unique(array_filter($fans_arr));
if (empty($fans_arr)||count($fans_arr)<1){
    echo '<div class="empty-collection">你还没有粉丝</div>';
}else{
    $valid_fans = []; // 创建一个新数组存储有效粉丝ID
    ...
    if ($p == 1) {
        ...
        $pageindex = '<a class="page-button nocopy" href="?type=6">首页</a>'; // 首页按钮
    }

    if ($p == $total_pages || $total_pages < 1) {
        $pagebody = '<a class="page-no-button nocopy">尾页</a>';
    } else {
        $pagebody = '<a class="page-button nocopy" href="?type=6&p='.$total_pages.'">尾页</a>';
    }

    if ($total_pages > 1 && $p < $total_pages) {
        $exit = $p + 1;
        $pageexit = '<a class="page-button nocopy" href="?type=6&p='.$exit.'">下一页</a>';
    } else {
    ...
    if ($p <= $total_pages && $p > 1) {
        $exitup = $p - 1;
        $pageup = '<a class="page-button nocopy" href="?type=6&p='.$exitup.'">上一页</a>';
    } else {
