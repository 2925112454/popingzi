document.addEventListener('DOMContentLoaded', function() {
  ...
  const upnewnetimg=document.getElementById('upnewnetimg');
  const body = document.querySelector('body');

  const filesizekb = 1024; // 限制文件上传大小，单位kb
  const allowedFileTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp','image/avif','video/mp4','image/svg+xml','application/x-mpegURL','image/x-icon','video/mpeg']; // 允许上传的文件类型
...
const friendlyFileType = mimeTypeMap[fileType] || mimeTypeMap['default'] || (filehz + wenjian);
  ...
  const uploadButton = document.getElementById('uplogoimg');
  const butloadButton = document.getElementById('butlogoimg');
        ...
        $.ajax({    
          ...
          success: function(data) { // data已经是解析后的对象  
              try {    
                  if (data.code === 200) {  
                    if (Typeup === 1) {  
                      $('#toplogoimgerr').val(data.url);
                      $('#logoimgnew').attr('src', URL.createObjectURL(lastDroppedFile));
                  } else if (Typeup === 2) {  
                      $('#butlogoimgerr').val(data.url); 
                     $('#butlogoimgnew').attr('src',URL.createObjectURL(lastDroppedFile));
                  }else if (Typeup === 3) {  
                    $('#upnewnetimgerr').val(data.url); 
                }
                  fileInput.value = '';
                  lastDroppedFile = null;
                  updateFileNameDisplay();  
                  handleFilePreview(null);
                  showError('<span class="fileyes"><i class="fa fa-check-square-o"></i>文件上传成功</span>');
  ...
  if (uploadButton && butloadButton && uploadOverlay && closeUploadOverlay && fileInput && fileInfo && fileTypeimg && openUploadOverlay) {
    uploadButton.addEventListener('click', topshowOverlay);
    butloadButton.addEventListener('click', butshowOverlay);
    upnewnetimg.addEventListener('click', netshowOverlay);
