...
function getUrlParam(name) {
  ...
  return null;
    ...
    const noticeform = document.getElementById('wordform');//获取表单
    const noticesubmit = document.getElementById('newwordsubmit');//获取提交按钮
    if (noticeform&&noticesubmit) {
        noticesubmit.addEventListener('click', function(event) { 
            event.preventDefault();
            const noticetite = noticeform.elements.rowhead.value;//获取标题
            const noticeimg = noticeform.elements.rowimg.value;//获取封面
            const noticetxt = tinymce.get('rowtext').getContent();;//获取内容
            const noticetop = parseInt(noticeform.elements.rowtop.value);//获取置顶,1不置顶，2置顶
            const textid = parseInt(getUrlParam('sid'));


            if(noticetite&&noticetxt){
                if(noticetop==1||noticetop==2){

                    if(noticeimg){
                        if(!isValidImageUrl(noticeimg)){
                            alert('<font>(｡ŏ_ŏ)</font> 封面图片格式错误！');
                            return;
                        }
                    }

                    if(!textid||textid<1||!Number.isInteger(textid)){
                      alert('<font>(｡ŏ_ŏ)</font> 错误操作！');
                      return;
                    }

                    console.log(noticetite);
                    $.ajax({
                      url: '/inc/noticeform.php', // 请求地址
                      type: 'POST',   // 请求类型
                      data: {
                      id: textid,
                      text: noticetxt,
                      ...
                      success: function(not) { // 成功回调函数
                        if(top == 500){
                          alert("<font>(｡ŏ_ŏ)</font> 错误操作！");
                        }else if (not == 200){
                          alert("<font>(｡ŏ_ŏ)</font> 修改成功，正在跳转！");
                              setTimeout(function(){
                              window.location.href = "popingzi.php?type=9";
                          },2000); 
                        }else if (not == 404){
                          alert("<font>(｡ŏ_ŏ)</font> 公告不存在！");
                        }else{
...
/* 上传照片 */
const newworduploadimg = document.getElementById('newworduploadimg');//获取上传按钮
if(newworduploadimg){
