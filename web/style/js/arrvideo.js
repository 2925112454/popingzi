...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var newarrvideo = document.getElementById('newarrimg');
    var rowvideoarr = document.getElementById('rowimgarr');
    var xcdesssvideo = document.getElementById('xcdesss');
    var videoeye = document.getElementById('imgeye');
    var suffix = /\.(mp4|m3u8|3gp|webm|ogg|mp3|wav)?$/i;
...
function handleDrop(e) {
    ...
    if (draggedBox && targetBox && draggedBox !== targetBox) {
        ...
        updateArrayOrder();
        // 保存到本地存储
        localStorage.setItem('video', JSON.stringify(vadeoarr));
    }
...
function updateArrayOrder() {
    const boxes = Array.from(videoeye.children);
    vadeoarr = boxes.map(box => box.dataset.video);
}

/**********拖动改变顺序功能实现模块*************/

    if (localStorage.getItem('video')!=''&&localStorage.getItem('video')!='null'&&localStorage.getItem('video')!=null&&localStorage.getItem('video')!='undefined'&&localStorage.getItem('video')!=undefined){
        var confirmResult = confirm("检测到上次未完成的视频，是否继续使用？");
        if (confirmResult) {
            vadeoarr=JSON.parse(localStorage.getItem('video'));
            xcdesssvideo.style.display = 'flex';
            //将vadeoarr中的视频输出到预览
            vadeoarr.forEach(function(videoUrl) {
            ...
            localStorage.setItem('video', '');//清空本地存储
            localStorage.removeItem('video');//删除本地存储
        }
    }
    ...
    if (newarrvideo && rowvideoarr && xcdesssvideo && videoeye) {
        ...
        if (rowvideoarrvalue) {
         ...
         // 检查视频地址是否已存在于数组中
        if (vadeoarr.includes(rowvideoarrvalue)) {
        ...
        if (!suffix.test(rowvideoarrvalue)) {
            ...
            if (!rowvideoarrvalue.startsWith('http://') && !rowvideoarrvalue.startsWith('https://')) {
                ...
                initializeNewVideo(videobox.querySelector('.video-item'));
                // 存储视频地址
                vadeoarr.push(rowvideoarrvalue);
                localStorage.setItem('video', JSON.stringify(vadeoarr));
                rowvideoarr.value='';
...
function deletevideoxx(value) {
    ...
    if (clickedBox) {
        ...
        const index = vadeoarr.indexOf(value);
        if (index > -1) {
            vadeoarr.splice(index, 1);
            localStorage.setItem('video', JSON.stringify(vadeoarr));
        }

        // 检查是否还有其他视频
        const remainingVideos = document.querySelectorAll('.videobox');
        if (remainingVideos.length < 1 || localStorage.getItem('video') == null || vadeoarr.length < 1 || localStorage.getItem('video') == '[]') {
...
document.addEventListener('DOMContentLoaded', function () {
 var submitButton = document.getElementById('newwordsubmit');//获取提交按钮
 var videoform=document.getElementById('wordform');//表单
 if (submitButton){
    submitButton.addEventListener('click', function (e) {
        ...
        const vadeoarrtext= vadeoarr.join(",");//将数组转换为字符串
        submitButton.style.pointerEvents="none";
        submitButton.style.opacity=0.5;
        submitButton.textContent="正在提交...";
                    //ajax提交表单
                    $.ajax({
                        url: '/api/upvideoall.php', // 请求地址
                        type: 'POST',   // 请求类型
                        data: {
                        tag: rowtag,//标签
                        title: newrowhead,//标题
                        ...
                        txttop:imgtexttop,//摘要位置
                                    ...
                                    success: function(all) { // 成功回调函数
                                        if(all == 500){
                                            ...
                                            alert("<font>(｡ŏ_ŏ)</font> 标题或视频不能为空！");
                                            resetSubmitBtn();
                                        }else if(all == 400){
                                            alert("<font>(｡ŏ_ŏ)</font> 标签格式不正确！");
                                            resetSubmitBtn();
                                        }else if(all == 405){
                                            alert("<font>(｡ŏ_ŏ)</font> 视频存在非法格式！");
                                            resetSubmitBtn();
                                        }else if(all == 401){
