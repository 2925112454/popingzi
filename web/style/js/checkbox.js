document.addEventListener('DOMContentLoaded', function() {
    ...
    const rowdel = document.querySelectorAll('.rowdel');  //获取所有类名为rowdel的删除按钮
    ...
    if (allcheckboxexe){
        allcheckboxexe.addEventListener('click', function(e) {
         ...
         if (checkedIdsx.length > 0) {
             ...
             // 弹出输入框确认
             const inputConfirmation = prompt('请输入状态(四选一)：发布、撤销、待审核、不通过');
             if (inputConfirmation !== null&&inputConfirmation!=''&&inputConfirmation!=' '&&inputConfirmation!==''&&inputConfirmation!==undefined&&!isPositiveInteger(inputConfirmation)) {
                ...
                if (inputConfirmation=='发布'||inputConfirmation=='撤销'||inputConfirmation=='待审核'||inputConfirmation=='不通过'){

                    let inputConfirmationmun=1;
                    if (inputConfirmation=='待审核') {
                        inputConfirmationmun=1;
                    }else if(inputConfirmation=='不通过'){
                        inputConfirmationmun=2;
                    }else if(inputConfirmation=='撤销'){
                        inputConfirmationmun=3;
                    }else if(inputConfirmation=='发布'){
                        inputConfirmationmun=4;
                    }else{
                        inputConfirmationmun=1;
                    }
                   const checkedIdsxString = checkedIdsx.join(',');//将数组转换为字符串
                    $.ajax({
                        url: '/inc/allexirow.php', // 请求地址
                        type: 'POST',   // 请求类型
                        data: {
