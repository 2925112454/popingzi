document.addEventListener('DOMContentLoaded', function() {
    var indexselect = document.getElementById('rowtype'); // 一级分类 select  
    var rowifselect = document.getElementById('rowif'); // 二级分类 select（这个变量在示例中未使用）
    if (typeof listfllinkid !== "undefined") {
      if (listfllinkid==""||listfllinkid=="0"||listfllinkid==undefined||listfllinkid=="null"||listfllinkid==0||listfllinkid==null){
        var indexvaluex = 1; // 初始化 indexvaluex 的值为 1 
      }else{
        var indexvaluex = listfllinkid;
      }
    }else{
      var indexvaluex = 1;
    }
  
    if (typeof listflid !== "undefined") {
      if (listflid==""||listfllinkid==undefined||listfllinkid=="null"||listfllinkid<0||listfllinkid==null){
        var  rowifvaluexfl = 0;
      }else{
        var  rowifvaluexfl = listflid;
      }
    }else{
      var  rowifvaluexfl = 0;
    }

    if(indexselect&&rowifselect){
        indexselect.onchange = function() {  
            var indexvalue = this.value; // 获取选中的值  
            indexvaluex = indexvalue; // 更新 indexvaluex 的值  
            $.ajax({//ajax获取对应二级分类,json格式
                url: '/inc/select.php', // 请求地址
                type: 'POST',   // 请求类型
                data: {
                    id: indexvaluex, // 一级分类id
                },
                dataType: 'json',
                success: function(datajs) {
                    var html = '';
                    for (var i = 0; i < datajs.length; i++) {
                      html += '<option value="' + datajs[i].id + '">' + datajs[i].name + '</option>';
                    }
                    rowifselect.innerHTML = html;
                },
        ...
        $.ajax({//ajax获取对应二级分类,json格式
            ...
            success: function(datajs) {
                var html = '';
                for (var i = 0; i < datajs.length; i++) {
                  if (rowifvaluexfl==datajs[i].id){
                    html += '<option value="' + datajs[i].id + '" selected>' + datajs[i].name + '</option>';
                  }else{
                    html += '<option value="' + datajs[i].id + '">' + datajs[i].name + '</option>';
                  }
...
if(newworduploadimg){
    ...
    document.querySelector('.custom-file-upload button').addEventListener('click', function() {
      ...
      const allowedFileTypes = ['image/jpeg', 'image/png', 'image/gif', 'video/mp4','image/svg+xml','application/x-mpegURL','image/x-icon','video/mpeg','image/webp','image/avif']; // 允许上传的文件类型
