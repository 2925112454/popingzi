...
CREATE TABLE `ppz_subcomm`  (
  ...
  `comm_ip` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '评论IP地址',
  `comm_time` timestamp NULL DEFAULT CURRENT_TIMESTAMP COMMENT '评论时间',
  `comm_top` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL COMMENT '评论点赞数量',
...
CREATE TABLE `ppz_subject`  (
  ...
  `ip` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '发表IP地址',
  `yes` int(11) NULL DEFAULT 1 COMMENT '状态，1待审核，2违规，3通过',
  `no` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL COMMENT '违规说明，状态2时有效',
  `quote` text CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL COMMENT '引用文章或话题的ID，多个ID用英文逗号进行分割，话题ID单独用{}包裹，如{1}',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = MyISAM AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Dynamic;
...
-- ----------------------------
-- Function structure for REGEXP_REPLACE (适配MySQL5.7)
-- ----------------------------
...
BEGIN
    ...
    -- MySQL5.7 无内置REGEXP_REPLACE，此处实现简单的字符串替换（按分隔符|匹配）
    -- 如需完整正则替换，需借助自定义逻辑或第三方插件
    WHILE i <= len DO
        IF temp REGEXP pattern THEN
            SET temp = REPLACE(temp, SUBSTRING(temp, REGEXP_INSTR(temp, pattern), CHAR_LENGTH(pattern)), replace_text);
        END IF;
        SET i = i + 1;
    END WHILE;
