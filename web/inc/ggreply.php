...
$text_len=mb_strlen($text,"utf-8");
    ...
    if ($text_len > 90){
        ...
        if (empty($text)){ //判断text是否为空
            ...
            $sql = "select * from ppz_ggcommentary where binary plid = $id";//查询数据库，判断评论
        ...
        if ($uban==1){
                     ...
                     $rowsqlnew = "select * from ppz_ggreply where binary repadmin = $uid  ORDER BY repid DESC LIMIT 1";//获取回复者最新的一条数据
                 ...
                 if($new===true){
                    ...
                    $sqlmun = "select * from ppz_ggreply  WHERE binary repplid=$id && binary repadmin = $uid";//获取回复
                    ...
                    if(mysqli_num_rows($retvalun) >= 3){//判断是否超过3条回复
                        ...
                        // 创建预处理语句    
                    $stmt = $conn->prepare("INSERT INTO ppz_ggreply (repplid,reptext,repadmin,repip) VALUES (?,?,?,?)");    
                    if ($stmt === false) {  
