...
// ========== 查询文章信息==========
$set_sql = "SELECT id, admin, text FROM ppz_subject WHERE BINARY id = ? AND admin = ?";
...
// 绑定参数（ID和UID均为整数）
$set_stmt->bind_param("ii", $id, $login_uid);
$set_stmt->execute();
...
// ========== 删除文章 ==========
$del_art_sql = "DELETE FROM ppz_subject WHERE id = ? AND admin = ?";
$del_art_stmt = $conn->prepare($del_art_sql);
if (!$del_art_stmt) {
    outputError(500);
}
$del_art_stmt->bind_param("ii", $id, $login_uid);
