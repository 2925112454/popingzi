...
/* 表单提交 */
    ...
    if(subpost&&subbtn&&submsg){
        subbtn.onclick=function (event){
            ...
            if (sub_title == "") {
                msg('错误：标题不能为空！',submsg,500,subbtn);
                return false;
            }

            if (sub_title.length > 180) {
                msg('错误：标题不能超过180个字符！',submsg,500,subbtn);
                return false;
            }

            if (sub_tag == "" || sub_tag < 1 || !isPositiveInteger(sub_tag)) {
                msg('错误：标签参数不正确！',submsg,500,subbtn);
                return false;
            }

            if (sub_content == "" || isEmptyValue(extractPlainTextFromHtml(sub_content))) {
                msg('错误：内容不能为空！',submsg,500,subbtn);
                return false;
            }

            if (!checkAllowedHtmlTags(sub_content)){
                msg('错误：非法操作！',submsg,500,subbtn);
                return false;
            }
                    ...
                    quote:quote_idsx,
                            ...
                            success: function(response) { // 成功回调函数
                                if(response == 500){
                                    ...
                                    msg('发表成功，正在跳转页面……',submsg,200);
                                }else if(response == 400){
                                    msg('错误：每天最多可发表'+submun+'条',submsg,500);
                                }else if(response == 600){
                                    msg('错误：发表失败',submsg,500);
                                    subbtn.style.opacity = "1";
                                    subbtn.innerHTML = "提交";
                                    subbtn.disabled = false;
